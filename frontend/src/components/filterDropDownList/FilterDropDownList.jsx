...
import closeDDF from '../../assets/img/remove-btn-filterdropdownlist.svg'
...
const FilterDropDownList = ({ items, onSelect, isClear, isClearDone }) => {
  ...
  const handleItemsSelection = (item) => {
    ...
    const lastSelectedItem = newSelectedItems[newSelectedItems.length - 1];
    const itemsButtons = listRef.current.querySelectorAll(
      ".filterdropdownlist-list__item"
    );
    itemsButtons.forEach((button) => {
      if (button.textContent === lastSelectedItem) {
        button.classList.add("filterdropdownlist-list__item-last-selected");
      } else {
        button.classList.remove("filterdropdownlist-list__item-last-selected");
      }
    });
  };
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      document.removeEventListener(
        "mousedown",
        handleDropddownListOutsideClick
      );
    }
  ...
  return (
    <div className="filterdropdownlist">
      <div
        className="filterdropdownlist__container"
        ref={containerRef}
        onClick={() => {
          setIsOpen(!isOpen);
        }}
        ...
        <div className="filterdropdownlist-list" ref={listRef}>
          <div className="filterdropdownlist-list__content">
            {[
              ...
              <button
                key={item}
                className={`filterdropdownlist-list__item ${
                  selectedItems.includes(item)
                    ? "filterdropdownlist-list__item-selected"
                    : ""
                } ${
                  item === selectedItems[selectedItems.length - 1]
                    ? "filterdropdownlist-list__item-last-selected"
                    : ""
                }`}
